...
FACTS = [
    ...
    {
        "fact": "the daily gemini budget is 240 calls per UTC day",
...
def _ask(question: str) -> str:
    ...
    if not lines:
        return ""
    # aegis-cli echoes the input prompt first; drop it if present.
...
def main() -> int:
    ...
    for item in FACTS:
        ...
        score = _score(reply, item["expected"])
        results.append({
            "fact": item["fact"],
    ...
    for r in results:
        ...
        print(f"  {marker} {r['question']!r} -> {reply_short!r}")
